...
class DebianPackager(object):
    ...
    def CompileRelease(self, repo_settings):
        ...
        release_file += "Description: " + \
            repo_settings['description'].replace(
                "\n\n", "\n .\n ").replace("\n", "\n ") + "\n"
    ...
    def CompileControl(self, tweak_data, repo_settings):
        ...
        try:
            if tweak_data['tagline']:
                ...
                control_file += "Description: " + \
                    tweak_data['tagline'].replace(
                        "\n\n", "\n .\n ").replace("\n", "\n ") + "\n"
        except Exception:
        ...
        try:
            if tweak_data['pre_dependencies']:
                control_file += "Pre-Depends: " + \
                    tweak_data['pre_dependencies'] + "\n"
        except Exception:
            pass
        ...
        try:
            ...
            control_file += "Depends: firmware (>=" + \
                tweak_data['works_min'] + ")\n"
        ...
        try:
            if tweak_data['build_depends']:
                control_file += "Build-Depends: " + \
                    tweak_data['build_depends'] + "\n"
        except Exception:
            pass

        try:
            if tweak_data['recommends']:
                control_file += "Recommends: " + \
                    tweak_data['recommends'] + "\n"
        except Exception:
            pass
        ...
        try:
            if tweak_data['tags']:
                control_file += "Tags: compatible_min::ios" + \
                    tweak_data['works_min'] + ", compatible_max::ios" + \
                    tweak_data['works_max'] + ", " + tweak_data['tags'] + "\n"
        except Exception:
            control_file += "Tags: compatible_min::ios" + \
                tweak_data['works_min'] + ", compatible_max::ios" + \
        ...
        try:
            if tweak_data['sponsor']:
                try:
                    ...
                    control_file += "Sponsor: " + \
                        tweak_data['sponsor']['name'] + "\n"
        except Exception:
            pass
        ...
        for file_name in os.listdir(self.root + "temp/" + bundle_id):
            if file_name.endswith(".deb"):
                ...
                if Dpkg.compare_versions(recorded_version, deb.version) == -1:
                    ...
                    with open(self.root + "Packages/" + package_name + "/silica_data/index.json", "r") as content_file:
                        ...
                        print(
                            "Due to some less-than-ideal design decisions with Silica, for the changelog to show")
                        print(
                            "up, you're going to have to run Silica again. Yes, I know this is annoying, and a proper")
                        print(
                            "solution is in the works, but the under-the-hood changes that'll be needed to fix")
                        print(
                            "it properly would require a rewrite [see issue #22].\n")
                        print("I'm deeply sorry about this.\n    - Shuga.\n")

                        # Get human-readable folder name
                        folder = PackageLister.BundleIdToDirName(
                            self, bundle_id)
                        ...
                        os.remove(self.root + "Packages/" + folder +
                                  "/silica_data/scripts/control")
                        if not os.listdir(self.root + "Packages/" + folder + "/silica_data/scripts/"):
                            os.rmdir(self.root + "Packages/" +
                                     folder + "/silica_data/scripts/")
            ...
            try:
                ...
                if docs_deb.version == recorded_version:
                    shutil.copy(self.root + "docs/pkg/" + bundle_id +
                                ".deb", self.root + "temp/" + bundle_id + ".deb")
                    call_result = 0
                else:
                    # Sneaky swap.
                    call_result = call(["dpkg-deb", "-b", "-Zgzip", self.root +
                                       "temp/" + bundle_id], cwd=self.root + "temp/")  # Compile DEB
            except:
        ...
        for folder in os.listdir(self.root + "Packages"):
            if folder.lower() != ".ds_store":
                if not os.path.isdir(self.root + "Packages/" + folder + "/silica_data"):
                    print("It seems like the package \"" + folder +
                    ...
                    if is_deb:
                        ...
                        try:
                            ...
                            output['Architecture'] = input(
                                "What is the Architecture(iphoneos-arm) of the package? ")
                        try:
                            output['tagline'] = deb.headers['Description']
                        except Exception:
                            output['tagline'] = input(
                                "What is a brief description of the package? ")
                        try:
                        ...
                        try:
                            ...
                            output['developer']['name'] = remove_email_regex.sub(
                                "", deb.headers['Author'])
                        except Exception:
                            output['developer']['name'] = input("Who originally made this package? This may be"
                                                                " your name. ")
                        output['developer']['email'] = input(
                            "What is the original author's email address? ")
                        try:
                            remove_email_regex = re.compile('<.*?>')
                            output['maintainer']['name'] = remove_email_regex.sub(
                                "", deb.headers['Maintainer'])
                        except Exception:
                        ...
                        output['works_min'] = input(
                            "What is the lowest iOS version the package works on? ")
                        
                        if output['works_min'] == "":
                            output['works_min'] = "15.0"
                        
                        output['works_max'] = input(
                            "What is the highest iOS version the package works on? ")
                        output['featured'] = input(
                            "Should this package be featured on your repo? (true/false) ")
                        ...
                        # Extract Control file and scripts from DEB
                        suc = DpkgPy.control_extract(self, deb_path, self.root + "Packages/" + folder +
                                                     "/silica_data/scripts/")
                        print(suc)
                        # Remove the Control; it's not needed.
                        os.remove(self.root + "Packages/" + folder +
                        ...
                        # Ask for name
                        output['name'] = input(
                            "What should we name this package? ")
                        # Automatically generate a bundle ID from the package name.
                        domain_breakup = repo_settings['cname'].split(".")[
                            ::-1]
                        only_alpha_regex = re.compile('[^a-zA-Z]')
                        machine_safe_name = only_alpha_regex.sub(
                            "", output['name']).lower()
                        output['bundle_id'] = ".".join(
                            str(x) for x in domain_breakup) + "." + machine_safe_name
                        output['tagline'] = input(
                            "What is a brief description of the package? ")
                        output['homepage'] = "https://" + \
                            repo_settings['cname']
                        # I could potentially default this to what is in settings.json but attribution may be an issue.
                        output['developer']['name'] = input(
                            "Who made this package? This is likely your name. ")
                        output['developer']['email'] = input(
                        ...
                        output['featured'] = input(
                            "Should this package be featured on your repo? (true/false) ")
                    PackageLister.CreateFolder(
                        self, "Packages/" + folder + "/silica_data/")
                    PackageLister.CreateFile(
                        self, "Packages/" + folder + "/silica_data/index.json", json.dumps(output))
    ...
    def CompilePackages(self):
        ...
        """
        # TODO: Update DpkgPy to generate DEB files without dependencies (for improved win32 support)
        call(["dpkg-scanpackages", "-m", "."], cwd=self.root +
             "docs/", stdout=open(self.root + "docs/Packages", "w"))
            ...
            with open(self.root + "docs/Release", "a") as text_file:
                ...
                try:
                    if repo_settings['enable_gpg'].lower() == "true":
                        ...
                        key = "Silica MobileAPT Repository"
                        call(["gpg", "-abs", "-u", key, "-o", "Release.gpg",
                             "Release"], cwd=self.root + "docs/")
